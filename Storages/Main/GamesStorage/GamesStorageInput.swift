...
public protocol GamesStorageInput {
    ...
    func addGamesStats(stats: [StatValue], achievements: [AchievementValue], for game: Game, user: User, completion: (() -> Void)?)
    func addGamesStats(stats: [StatValue], achievements: [AchievementValue], for game: Game, schemaStats: [Stat], schemaAchievements: [Achievement], user: User, completion: (() -> Void)?)
